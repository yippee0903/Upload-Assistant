...
class TestValidateReusedImageHosts:
    ...
    def test_relevant_tracker_is_validated(self, monkeypatch):
        from src.rehostimages import RehostImagesManager

        calls: list[tuple] = []

        async def fake_check_hosts(self, meta, tracker, img_host_index, approved_image_hosts):
            calls.append((meta, tracker, approved_image_hosts))
            meta["V3X_images_key"] = [{"img_url": "https://approved.example/a.png"}]

        monkeypatch.setattr(RehostImagesManager, "check_hosts", fake_check_hosts)
