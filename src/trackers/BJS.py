...
class BJS:
    ...
    def _extract_upload_params(self, meta: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"is_tv_pack": is_tv_pack, "upload_season_num": upload_season_num, "upload_episode_num": upload_episode_num, "upload_resolution": upload_resolution}
    ...
    def _should_process_torrent(
        self, row: Tag, current_season: str, current_resolution: str, params: dict[str, Any], episode_found_on_page: bool, meta: dict[str, Any]
    ) -> tuple[bool, bool]:
        link = row.find("a", onclick=re.compile(r"loadIfNeeded\("))
    ...
    async def _fetch_torrent_content(self, task_info: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"success": False, "error": last_error, "task_info": task_info}
    ...
    async def _process_ajax_responses(self, ajax_tasks: list[dict[str, Any]], params: dict[str, Any]) -> list[dict[str, str]]:
        ...
        ajax_results = await asyncio.gather(*[self._fetch_torrent_content(task) for task in ajax_tasks], return_exceptions=True)
        ...
        for result in ajax_results:
            ...
            item_name = self._extract_item_name(soup_obj, description_text, params["is_tv_pack"], process_folder_name)
    ...
    async def search_existing(self, meta: dict[str, Any], _) -> list[dict[str, str]]:
        ...
        try:
            ...
            if meta["category"] == "TV" and not params["is_tv_pack"] and params["upload_season_num"] and params["upload_episode_num"]:
                episode_found_on_page = self._check_episode_on_page(torrent_details_table, params["upload_season_num"], params["upload_episode_num"])
            ...
            for row in torrent_details_table.find_all("tr"):
                ...
                if isinstance(row_classes, list):
                    ...
                    should_process, process_folder_name = self._should_process_torrent(
                        row, current_season_on_page, current_resolution_on_page, params, episode_found_on_page, meta
                    )
