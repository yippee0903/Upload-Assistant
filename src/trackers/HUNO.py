...
class HUNO(UNIT3D):
    ...
    async def get_description(self, meta: dict[str, Any]) -> dict[str, str]:
        ...
        return {
            "description": await DescriptionBuilder(self.tracker, self.config).unit3d_edit_desc(meta, image_list=image_list, approved_image_hosts=self.approved_image_hosts)
        }
    ...
    async def get_name(self, meta: dict[str, Any]) -> dict[str, str]:
        ...
        if meta["category"] == "MOVIE":  # MOVIE SPECIFIC
            ...
            if type == "DISC":  # Disk
                ...
                name = (
                    f"{title} ({year}) {season}{episode} {edition} ({resolution} {three_d} {source} {hybrid} REMUX {video_codec} {hdr} {hfr} {audio} {tag}) {repack}"  # SOURCE
                )
            elif type == "REMUX" and source in ("PAL DVD", "NTSC DVD", "DVD"):  # DVD Remux
