            ...
            for line in mediainfo_text.splitlines():
                ...
                if ":" in line:
                    ...
                    if (
                        ...
                        or current_section == "video"
                        and property_name in video_fields
                        or current_section == "audio"
    ...
    async def get_bdmv_mediainfo(self, meta: dict[str, Any], remove: Optional[list[str]] = None, char_limit: int = 0) -> str:
        ...
        if meta.get("is_disc") == "BDMV":
            ...
            if not os.path.isfile(mi_path):
                ...
                await exportInfo(path, False, meta["uuid"], meta["base_dir"], is_dvd=False, debug=meta.get("debug", False))
            ...
            if char_limit and len(mediainfo) > char_limit:
                ...
                if items:
                    ...
                    if largest_m2ts:
                        ...
                        await exportInfo(largest_m2ts, False, meta["uuid"], meta["base_dir"], is_dvd=False, debug=meta.get("debug", False))
