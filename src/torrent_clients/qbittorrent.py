...
class QbittorrentClientMixin:
    ...
    async def get_ptp_from_hash_qbit(self, meta: dict[str, Any], client: dict[str, Any], pathed: bool = False) -> dict[str, Any]:
        if not await self._confirm_proceed_if_offline(client, meta):
            return meta
        ...
        try:
            ...
            console.print("[yellow]qBittorrent proxy timed out while fetching torrent properties; skipping.")
            if qbt_session:
                await qbt_session.close()
            return meta
        except Exception as e:
            console.print(f"[bold red]Error getting torrent properties: {e}")
            if qbt_session:
                await qbt_session.close()
            return meta
        found = False
    ...
    async def _confirm_proceed_if_offline(self, client: dict[str, Any], meta: dict[str, Any]) -> bool:
        ...
        Returns True to proceed with the operation, False to skip it.
        Uses a per-process cache so the health-check ping is only made once per client URL.
        """
        proxy_url = client.get("qui_proxy_url", "").strip()
        cache_key = proxy_url if proxy_url else f"{client.get('qbit_url')}:{client.get('qbit_port')}"

        if cache_key not in _qbit_health_cache:
            _qbit_health_cache[cache_key] = await self._check_qbit_reachable(client)
    ...
    async def _proxy_get_with_retry(
        ...
        Returns ``(status_code, json_body)``.  ``json_body`` is ``None`` when the
        response body cannot be parsed as JSON.  Raises ``asyncio.TimeoutError``
            ...
            try:
                async with session.get(url, params=params) as response:
                    if response.status in (502, 503, 504) and attempt < max_retries:
                        console.print(f"[yellow]Proxy returned {response.status} (attempt {attempt + 1}/{max_retries + 1}), retrying in {retry_delay:.0f}s...")
                        await asyncio.sleep(retry_delay)
                        continue
                    try:
                ...
                if attempt < max_retries:
                    console.print(f"[yellow]Proxy request timed out (attempt {attempt + 1}/{max_retries + 1}), retrying in {retry_delay:.0f}s...")
                    await asyncio.sleep(retry_delay)
        raise last_exc
    ...
    async def init_qbittorrent_client(self, client: dict[str, Any]) -> Optional[qbittorrentapi.Client]:
        ...
        # qbit_api_key (qBittorrent >= 5.2, stateless) takes precedence over user/password
        api_key = str(client.get("qbit_api_key") or "").strip()
        client_key = (client["qbit_url"], client["qbit_port"], f"APIKEY:{api_key}" if api_key else client.get("qbit_user", ""))
        async with qbittorrent_locks[client_key]:
            ...
            verify_cert = client.get("VERIFY_WEBUI_CERTIFICATE", True)
            if api_key:
                qbt_client = qbittorrentapi.Client(host=client["qbit_url"], port=client["qbit_port"], api_key=api_key, VERIFY_WEBUI_CERTIFICATE=verify_cert)
                login, label = qbt_client.app_version, "qBittorrent API key verification"
            else:
                qbt_client = qbittorrentapi.Client(
                    host=client["qbit_url"], port=client["qbit_port"], username=client.get("qbit_user"), password=client.get("qbit_pass"), VERIFY_WEBUI_CERTIFICATE=verify_cert
                )
                login, label = qbt_client.auth_log_in, "qBittorrent login"
            try:
                await self.retry_qbt_operation(lambda: asyncio.to_thread(login), label)
            except asyncio.TimeoutError:
                ...
                console.print("[bold red]Failed to connect to qBittorrent - check host/port" + ("/API key" if api_key else ""))
                return None
            else:
        ...
        try:
            ...
            try:
                if proxy_url:
                    ...
                    status, response_data = await self._proxy_get_with_retry(qbt_session, url)
                    if status == 200 and response_data is not None:
                        ...
                        if status == 404:
                            ...
                            console.print(f"[bold red]Failed to get torrents list via proxy: {status}")
                        return None
                else:
                    if qbt_client is None:
                ...
                console.print("[yellow]qBittorrent proxy timed out while searching for torrent; a new BASE.torrent will be created.")
                return None
            except Exception as e:
    ...
    def _resolve_src_and_save_path(
        ...
        if single_file and not meta.get("keep_folder") and not torrent_is_multi_file:
            ...
            # multi-file: the caller already set path = os.path.dirname(release_dir).
            # meta["path"] is always the release directory for multi-file releases.
            src = meta.get("path", "")
    ...
    async def qbittorrent(
        ...
        try:
            if proxy_url:
                ...
                async def _proxy_add() -> int:
                    fd = aiohttp.FormData()
                    fd.add_field("savepath", save_path)
                    fd.add_field("autoTMM", str(auto_management).lower())
                    fd.add_field("skip_checking", str(skip_checking).lower())
                    fd.add_field("paused", str(paused_on_add).lower())
                    fd.add_field("contentLayout", content_layout)
                    if qbt_category:
                        fd.add_field("category", qbt_category)
                    if tag:
                        fd.add_field("tags", tag)
                    fd.add_field("torrents", torrent.dump(), filename="torrent.torrent", content_type="application/x-bittorrent")
        ...
        for _ in range(timeout):
            try:
                if proxy_url:
                    if qbt_session is None:
                        raise RuntimeError("qbt_session cannot be None")
                    async with qbt_session.get(f"{qbt_proxy_url}/api/v2/torrents/info", params={"hashes": torrent.infohash}) as response:
                        if response.status == 200:
        ...
        if tracker in client.get("super_seed_trackers", []) and not cross:
            try:
                ...
                if proxy_url:
                    if qbt_session is None:
                        raise RuntimeError("qbt_session cannot be None")
                    async with qbt_session.post(f"{qbt_proxy_url}/api/v2/torrents/setSuperSeeding", data={"hashes": torrent.infohash, "value": "true"}) as response:
                        if response.status != 200:
    ...
    def _extract_tracker_matches(
        self, torrent: Any, tracker_patterns: dict[str, dict[str, str]], tracker_priority: list[str], has_working_tracker: bool, meta: dict[str, Any]
    ...
    async def _search_single_qbit_client(self, client_config: dict[str, Any], _content_path: str, meta: dict[str, Any], client_name: str) -> list[dict[str, Any]]:
        ...
        if not await self._confirm_proceed_if_offline(client_config, meta):
            return []
        qbt_session: Optional[aiohttp.ClientSession] = None
        qbt_client: Optional[qbittorrentapi.Client] = None
        qbt_proxy_url = ""
        ...
        try:
            tracker_patterns = {
                ...
                "cbr": {"url": "https://capybarabr.com", "pattern": r"/(\d+)$"},
                "emuw": {"url": "https://emuwarez.com", "pattern": r"/(\d+)$"},
                "gf": {"url": "https://generation-free.org", "pattern": r"/(\d+)$"},
                "itt": {"url": "https://itatorrents.xyz", "pattern": r"/(\d+)$"},
                "lcd": {"url": "https://locadora.cc", "pattern": r"/(\d+)$"},
                "ldu": {"url": "https://theldu.to", "pattern": r"/(\d+)$"},
                "lt": {"url": "https://lat-team.com", "pattern": r"/(\d+)$"},
                "nst": {"url": "https://nostradamus.foo", "pattern": r"/(\d+)$"},
                "pt": {"url": "https://portugas.org", "pattern": r"/(\d+)$"},
                "ptt": {"url": "https://polishtorrent.top", "pattern": r"/(\d+)$"},
                "r4e": {"url": "https://racing4everyone.eu", "pattern": r"/(\d+)$"},
                "ras": {"url": "https://rastastugan.org", "pattern": r"/(\d+)$"},
                "sam": {"url": "https://samaritano.cc", "pattern": r"/(\d+)$"},
                "shri": {"url": "https://shareisland.org", "pattern": r"/(\d+)$"},
                "tik": {"url": "https://cinematik.net", "pattern": r"/(\d+)$"},
                "tlz": {"url": "https://tlzdigital.com", "pattern": r"/(\d+)$"},
                "ttr": {"url": "https://torrenteros.org", "pattern": r"/(\d+)$"},
                "utp": {"url": "https://utp.to", "pattern": r"/(\d+)$"},
            }
                    ...
                    if not meta.get("base_torrent_created"):
                        ...
                        if not torrent_file_path:
                            ...
                            if proxy_url:
                                ...
                                try:
                                    torrent_file_content = await self.retry_qbt_operation(
                                        lambda: asyncio.to_thread(qbt_client.torrents_export, torrent_hash=torrent_hash), f"Export torrent {torrent_hash}"
                                    )
                                except (asyncio.TimeoutError, Exception) as e:
                                    console.print(f"[bold red]Failed to export .torrent for {torrent_hash}: {e}")
                                    torrent_file_content = None
                            ...
                            for torrent_match in matching_torrents[1:]:  # Skip the first one since we already tried it
                                ...
                                if not alt_torrent_file_path:
                                    ...
                                    if proxy_url:
                                        ...
                                        qbt_proxy_url = proxy_url.rstrip("/")
                                        try:
                                            async with qbt_session.post(f"{qbt_proxy_url}/api/v2/torrents/export", data={"hash": alt_torrent_hash}) as response:
                                                if response.status == 200:
...
async def create_cross_seed_links(meta: dict[str, Any], torrent: Torrent, tracker_dir: str, use_hardlink: bool, tracker: str = "") -> bool:
    ...
    for candidate in candidate_paths:
        ...
        try:
            ...
            size = None
        unique_candidates.append({"path": abs_candidate, "name": os.path.basename(abs_candidate).lower(), "size": size, "used": False})
