...
async def disc_screenshots(
    ...
    if meta.get("frame_overlay", False):
        ...
        frame_info_tasks_with_idx = [
            ...
            if not os.path.exists(f"{base_dir}/tmp/{folder_id}/{sanitized_filename}-{len(existing_screens) + i}.png") or meta.get("retake", False)
        ]
    ...
    if use_vs:
        ...
        async def capture_disc_with_semaphore(
            index: int, file: str, ss_time: str, image_path: str, keyframe: str, loglevel: str, hdr_tonemap: bool, meta: dict[str, Any]
...
async def capture_disc_task(
    ...
    try:
        ...
        info_command: Any = (
            cast(Any, ffmpeg)
            .input(file, ss=str(ss_time), skip_frame=keyframe)
...
async def dvd_screenshots(meta: dict[str, Any], disc_num: int, num_screens: int = 0, retry_cap: bool = False) -> None:
    screens = meta["screens"]
    if "image_list" not in meta:
        meta["image_list"] = []
    image_list_entries = cast(list[dict[str, Any]], meta["image_list"])
    ...
    if existing_images_count == num_screens and not meta.get("retake", False):
        ...
        if meta.get("frame_overlay", False):
            if meta["debug"]:
                console.print("[yellow]Getting frame information for overlays...")
            frame_info_tasks = [
                get_frame_info(input_files[i], ss_times[i], meta) for i in range(num_screens + 1) if not os.path.exists(image_paths[i]) or meta.get("retake", False)
            ]
...
async def screenshots(
    ...
    if meta.get("frame_overlay", False):
        ...
        frame_info_tasks_with_idx = [
            ...
            if not os.path.exists(f"{base_dir}/tmp/{folder_id}/{sanitized_filename}-{existing_images_count + i}.png") or meta.get("retake", False)
        ]
        frame_info_results = await asyncio.gather(*[task for _, task in frame_info_tasks_with_idx])
        meta["frame_info_map"] = {}
    ...
    try:
        ...
        capture_result_tuples: list[tuple[int, Optional[str]]] = [cast(tuple[int, Optional[str]], r) for r in results if isinstance(r, tuple)]
...
async def get_frame_info(path: str, ss_time: Union[str, float], meta: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if meta.get("debug", False):
            console.print(traceback.format_exc())
        return {"frame_type": "Unknown", "frame_number": int(float(ss_time) * meta.get("frame_rate", 24.0))}


async def check_libplacebo_compatibility(
    w_sar: float, h_sar: float, width: float, height: float, path: str, ss_time: str, image_path: str, loglevel: str, meta: dict[str, Any]
) -> tuple[bool, bool]:
    ...
    async def run_check(
        ...
        if try_libplacebo:
            ...
            vf_chain = f"zscale=transfer=linear,tonemap=tonemap={algorithm}:desat={desat},zscale=transfer=bt709,format=rgb24"
            info_cmd: Any = (
                cast(Any, ffmpeg).input(path, ss=str(ss_time)).output(test_image_path, vframes=1, vf=vf_chain, pix_fmt="rgb24").global_args("-y", "-loglevel", "quiet")
            )
...
class TakeScreensManager:
    ...
    async def capture_disc_task(
        self, index: int, file: str, ss_time: str, image_path: str, keyframe: str, loglevel: str, hdr_tonemap: bool, meta: dict[str, Any]
    ) -> Optional[tuple[int, str]]:
        return await capture_disc_task(index, file, ss_time, image_path, keyframe, loglevel, hdr_tonemap, meta)

    async def dvd_screenshots(self, meta: dict[str, Any], disc_num: int, num_screens: int = 0, retry_cap: bool = False) -> None:
        await dvd_screenshots(meta, disc_num, num_screens, retry_cap)

    async def capture_dvd_screenshot(self, task: tuple[int, str, str, str, dict[str, Any], float, float, float, float]) -> tuple[int, Optional[str]]:
    ...
    async def capture_screenshot(self, args: tuple[int, str, float, str, float, float, float, float, str, bool, dict[str, Any]]) -> Optional[tuple[int, Optional[str]]]:
        return await capture_screenshot(args)

    async def valid_ss_time(self, ss_times: list[str], num_screens: int, length: float, frame_rate: float, meta: dict[str, Any], retake: bool = False) -> list[str]:
    ...
    async def check_libplacebo_compatibility(
        self, w_sar: float, h_sar: float, width: float, height: float, path: str, ss_time: str, image_path: str, loglevel: str, meta: dict[str, Any]
    ) -> tuple[bool, bool]:
